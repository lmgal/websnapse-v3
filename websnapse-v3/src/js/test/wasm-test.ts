...
console.assert(model.getTransposedSpikingTransitionMatrix().every(
    (cell, i) => cell === correctTransposedTransitionMatrix[i], 'Incorrect transposed spiking transition matrix'
))
const correctConfigurationVector = [2, 3, 1, 0]
console.assert(model.getInitialConfigurationVector().every(
    (cell, i) => cell === correctConfigurationVector[i], 'Incorrect initial configuration vector'
))
const correctDelayVector = [0, 0, 0, 1, 0, 0, 1]
console.assert(model.getDelayVector().every(
    (cell, i) => cell === correctDelayVector[i], 'Incorrect delay vector'
))
